...
export const WidgetBox: React.SFC<{
    ...
    <div style={{border: 'black thin dotted',
        ...
        <Movable immediate onClick={stopPropagation} onMove={(deltaX, deltaY, isEnd) => onResizeWidget(widget.id, deltaX, deltaY, widget.width - deltaX, widget.height - deltaY, isEnd)}>
            <div style={{position: 'absolute', backgroundColor: 'white', left: '-4px', top: '-4px', width: '8px', height: '8px', border: 'black thin solid'}}></div>
        </Movable>
        <Movable immediate onClick={stopPropagation} onMove={(deltaX, deltaY, isEnd) => onResizeWidget(widget.id, 0, deltaY, widget.width + deltaX, widget.height - deltaY, isEnd)}>
        ...
        <Movable immediate onClick={stopPropagation} onMove={(deltaX, deltaY, isEnd) => onResizeWidget(widget.id, 0, 0, widget.width + deltaX, widget.height + deltaY, isEnd)}>
        ...
        <Movable immediate onClick={stopPropagation} onMove={(deltaX, deltaY, isEnd) =>onResizeWidget(widget.id, deltaX, 0, widget.width - deltaX, widget.height + deltaY, isEnd)}>
